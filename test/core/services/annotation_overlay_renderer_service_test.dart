...
void main() {
  group('AnnotationOverlayRendererService overlay transform', () {
    ...
    test('maps points linearly when viewport and video share aspect ratio', () {
      ...
      expect(transform.styleScaleFactor, closeTo(2.0, 0.001));
    });

    test('accounts for pillarbox offsets when mapping viewport points', () {
      final transform = service.createOverlayTransform(
        outputWidth: 1440,
        outputHeight: 1080,
        viewportWidth: 1920,
      ...
      expect(leftEdge.dx, closeTo(0.0, 0.001));
      expect(center.dx, closeTo(720.0, 0.001));
      expect(rightEdge.dx, closeTo(1440.0, 0.001));
      expect(leftBar.dx, lessThan(0.0));
